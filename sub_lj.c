...
void draw_colored_ellipse_precomp(double x, double y, double rx, double ry, double rgb[3])
{
    int i;
...
int generate_poisson_discs(t_point *point, int nmax, double xmin, double xmax, double ymin, double ymax, double dpoisson, int sphere)
    ...
    while ((n_p_active > 0)&&(npoints < nmax))
    {
        ...
        for (j=0; j<ncandidates; j++)
        {
            ...
            for (k=0; k<npoints; k++) if ((k!=i))
            {
                ...
                if (sphere)
                {
                    far = far*(dist_sphere(x, y, point[k].xc, point[k].yc) >= dpoisson);
                }
                else 
                {
...
{
    int i, j, k, n, ncirc0, n_p_active, ncandidates = PDISC_CANDIDATES, naccepted, nx; 
...
//                     printf("No candidates work, inactivate circle %i\n", i);
        ...
        case (C_POISSON_DISC_SPHERE):
        {
            point = (t_point *)malloc(NMAXOBSTACLES*sizeof(t_point));
            ncircles = generate_poisson_discs(point, NMAXCIRCLES, INITXMIN, INITXMAX, INITYMIN, INITYMAX, PDISC_DISTANCE*MU, 1);
            
            for (n=0; n<ncircles; n++)
            {
                    particles[n].xc = point[n].xc;   
                    particles[n].yc = point[n].yc;
                    particles[n].radius = MU;
                    particles[n].active = 1;
            }
            printf("Generated %i circles\n", ncircles);
...
//                 printf("i = %i, circlex = %.3lg, circley = %.3lg\n", i, particles[i].xc, particles[i].yc);
        ...
        case (C_POOL_TABLE):
        {
            for (i=1; i<NGRIDX; i++) for (j=0; j<i; j++)
            {
                r = 1.5/(double)NGRIDX;
                particles[ncircles].xc = INITXMIN + (double)i*r*(INITXMAX - INITXMIN);
                particles[ncircles].yc = 0.5*(INITYMIN + INITYMAX) + ((double)j - 0.5*(double)(i-1))*r*(INITYMAX - INITYMIN);
                particles[ncircles].radius = MU;
                particles[ncircles].active = 1;
                ncircles += 1;
            }
        ...
        case (C_SQUARE_SPHERE):
        {
            printf("initializing square-sphere pattern\n");
            fprintf(lj_log, "initializing square-sphere pattern\n");
            
            dx = (INITXMAX - INITXMIN)/((double)NGRIDX);
            dy = (INITYMAX - INITYMIN)/((double)NGRIDY);
            printf("dx = %.3lg, dy = %.3lg\n", dx, dy);
            fprintf(lj_log, "dx = %.3lg, dy = %.3lg\n", dx, dy);
            
            n = 0;
            for (j = 0; j < NGRIDY; j++)
            {
                y = INITYMIN + ((double)j + 0.5)*dy;
                nx = (int)((double)NGRIDX*sin(y));
                dx = (INITXMAX - INITXMIN)/(double)nx;
                for (i = 0; i < nx; i++)
                {
                    particles[n].xc = INITXMIN + ((double)i + 0.5)*dx;
                    particles[n].yc = y;
                    ...
                    printf("(x,y) = (%.3lg,%.3lg)\n", particles[n].xc, particles[n].yc);
                    fprintf(lj_log, "n = %i, (x,y) = (%.3lg,%.3lg)\n", n, particles[n].xc, particles[n].yc);
...
//             printf("\n");
    ...
    {
        ofacet[i].area0 = otriangle[ofacet[i].triangle[0]].area0;
    }
...
{
    int s1, s2, s3;
    double deltax2, deltay2, deltax3, deltay3;
    static double dx, dy;
    static int first = 1;
    ...
    deltax2 = -dx*(double)triangle.shiftx[1];
    deltay2 = -dy*(double)triangle.shifty[1];
    deltax3 = -dx*(double)triangle.shiftx[2];
    deltay3 = -dy*(double)triangle.shifty[2];
    
//     fprintf(lj_log, "delta = (%.3lg, %.3lg), (%.3lg, %.3lg)\n", deltax2, deltay2, deltax3, deltay3);
    
    return(vabs(triangle_area(obstacle[s1].xc, obstacle[s1].yc, obstacle[s2].xc + deltax2, obstacle[s2].yc + deltay2, obstacle[s3].xc + deltax3, obstacle[s3].yc + deltay3)));
}
...
{
    int i, j, n, m, p, q, shiftx, shifty, table[NMAX_OBSTACLE_NEIGHBOURS], neigh_table[NMAX_OBSTACLE_NEIGHBOURS];
    double dist, dist1, bdistx, bdisty, angle[NMAX_OBSTACLE_NEIGHBOURS], dist_table[NMAX_OBSTACLE_NEIGHBOURS], dx, dy;
    short int near_boundary, shiftx_table[NMAX_OBSTACLE_NEIGHBOURS], shifty_table[NMAX_OBSTACLE_NEIGHBOURS];
        ...
        for (j=i+1; j<nobstacles; j++) if (j != i)
        {
            if (BOUNDARY_COND == BC_PERIODIC)
            {
                ...
                for (p=-1; p<2; p++)
                    for (q=-1; q<2; q++)
                    {
                        dist1 = module2(obstacle[i].xc - obstacle[j].xc - (double)p*(XMAX-XMIN), obstacle[i].yc - obstacle[j].yc - (double)q*(YMAX - YMIN));
                        if (dist1 < dist) 
                        {
                            dist = dist1;
                            shiftx = p;
            ...
            if ((dist < OBSTACLE_COUPLING_DIST)&&(obstacle[i].nneighb < NMAX_OBSTACLE_NEIGHBOURS))
            {
                ...
                obstacle[i].shiftx[n] = -shiftx;
                obstacle[i].shifty[n] = -shifty;
                obstacle[i].nneighb++;
                
                n = obstacle[j].nneighb;
                obstacle[j].neighb[n] = i;
                obstacle[j].eqdist[n] = dist;
                obstacle[j].shiftx[n] = shiftx;
                obstacle[j].shifty[n] = shifty;
                obstacle[j].nneighb++;
            }
        }
        
//         fprintf(lj_log, "Obstacle %i at (%.3lg, %.3lg) has %i neighbours: \n", i, obstacle[i].xc, obstacle[i].yc, obstacle[i].nneighb);
//         for (j=0; j<obstacle[i].nneighb; j++)
//         {
//             fprintf(lj_log, "Neighbour %i at (%.3lg, %.3lg) has shift (%i, %i)\n", obstacle[i].neighb[j], obstacle[obstacle[i].neighb[j]].xc, obstacle[obstacle[i].neighb[j]].yc, obstacle[i].shiftx[j], obstacle[i].shifty[j]);
//             
//         }
...
//         printf("Obstacle %i has %i neighbours\t", i, obstacle[i].nneighb);
        ...
        for (j=0; j<n; j++)
        {
            ...
            dx = -(XMAX-XMIN)*obstacle[i].shiftx[j];
            dy = -(YMAX-YMIN)*obstacle[i].shifty[j];
            angle[j] = argument(obstacle[m].xc + dx - obstacle[i].xc, obstacle[m].yc + dy - obstacle[i].yc);
...
//         printf("\n");
        ...
        for (j=0; j<n; j++)
        {
            ...
            shiftx_table[j] = obstacle[i].shiftx[j];
            shifty_table[j] = obstacle[i].shifty[j];
        }
        for (j=0; j<n; j++)
        {
            obstacle[i].neighb[j] = neigh_table[table[j]];
            obstacle[i].eqdist[j] = dist_table[table[j]];
            obstacle[i].shiftx[j] = shiftx_table[table[j]] ;
            obstacle[i].shifty[j] = shifty_table[table[j]] ;
        }
...
//             printf("Building triangle %i\n", n_otriangles);
            for (j=0; j<n-1; j++) if (angle[j+1] - angle[j] < PI)
            {
                ...
                otriangle[n_otriangles].shiftx[0] = 0;
                otriangle[n_otriangles].shifty[0] = 0;
                otriangle[n_otriangles].shiftx[1] = obstacle[i].shiftx[j];
                otriangle[n_otriangles].shifty[1] = obstacle[i].shifty[j];
                otriangle[n_otriangles].shiftx[2] = obstacle[i].shiftx[j+1];
                otriangle[n_otriangles].shifty[2] = obstacle[i].shifty[j+1];
                n_otriangles++;
            }
            if (angle[0] - angle[n-1] + DPI < PI)
            {
                ...
                otriangle[n_otriangles].shiftx[0] = 0;
                otriangle[n_otriangles].shifty[0] = 0;
                otriangle[n_otriangles].shiftx[1] = obstacle[i].shiftx[n-1];
                otriangle[n_otriangles].shifty[1] = obstacle[i].shifty[n-1];
                otriangle[n_otriangles].shiftx[2] = obstacle[i].shiftx[0];
                otriangle[n_otriangles].shifty[2] = obstacle[i].shifty[0];
                n_otriangles++;
            }
        }
    }
    
//     fprintf(lj_log, "[init_obstacle_coupling]\n");
//     for (i=0; i<n_otriangles; i++)
//     {
//         fprintf(lj_log, "Triangle %i\n", i);
//         for (j=0; j<3; j++)
//         {
//             fprintf(lj_log, "Obstacle %i ", otriangle[i].i[j]);
//             fprintf(lj_log, "shiftx %i ", otriangle[i].shiftx[j]);
//             fprintf(lj_log, "shifty %i\n", otriangle[i].shifty[j]);            
//         }
//     }
    
    /* damp obstacles on boundary */
    for (i=0; i<nobstacles; i++)
    {
        for (j=0; j<obstacle[i].nneighb; j++)
            if ((obstacle[i].shiftx[j] != 0)||(obstacle[i].shifty[j] != 0))
                obstacle[i].damped = 1;
    }
    ...
    for (i=0; i<n_otriangles; i++)
    {
        ...
        fprintf(lj_log, "Triangle %i area0 %.3lg\n", i, otriangle[i].area0);
//         for (j=0; j<3; j++)
//         {
//             fprintf(lj_log, "Obstacle %i ", otriangle[i].i[j]);
//             fprintf(lj_log, "shiftx %i ", otriangle[i].shiftx[j]);
...
/* initialise circular obstacle configuration */
{
    ...
    switch (OBSTACLE_PATTERN) {
        ...
        {
            point = (t_point *)malloc(NMAXOBSTACLES*sizeof(t_point));
            nobstacles = generate_poisson_discs(point, NMAXOBSTACLES, OBSXMIN, OBSXMAX, OBSYMIN, OBSYMAX, OBSTACLE_PISC_DISTANCE, 0);
...
/* initialise linear obstacle configuration */
{
    ...
    switch (SEGMENT_PATTERN) {
        ...
        case (S_CYLINDRICAL):
        {
            segment[0].x1 = BCXMIN - 1.0;
            segment[0].y1 = BCYMAX;
            
            segment[1].x1 = BCXMIN - 1.0;
            segment[1].y1 = BCYMIN;

            segment[2].x1 = BCXMAX + 1.0;
            segment[2].y1 = BCYMIN;

            segment[3].x1 = BCXMAX + 1.0;
            segment[3].y1 = BCYMAX;
...
//             for (i=0; i<nsegments; i++) segment[i].group = 1;
        ...
        case (S_TREES_B):
        {
            for (i=0; i<NTREES; i++)
            {
                x = XMIN + ((double)i + 0.5 + 0.1*(double)rand()/(double)RAND_MAX)*(XMAX-XMIN)/((double)NTREES+0.7); 
                if (i == NTREES - 1) 
                    y = YMIN + OFSSET_TREES + SLOPE_TREES*(YMAX - YMIN);
                else if (i<NTREES/2) 
                    y = YMIN + OFSSET_TREES + SLOPE_TREES*((double)i+0.7)*(YMAX - YMIN);
                else 
                    y = YMIN + OFSSET_TREES + SLOPE_TREES*((double)(NTREES/2) + 0.7)*(YMAX - YMIN) - SLOPE_TREES_B*((double)(i - NTREES/2) + 0.7)*(YMAX - YMIN);
                add_tree_to_segments(x, y, LAMBDA*(1.0 +  0.4*(double)rand()/(double)RAND_MAX), lfoot[i], rfoot[i], segment, i+1);
            }
            /* add ground */
            segment[nsegments].x1 = XMIN;
            segment[nsegments].y1 = YMIN + OFSSET_TREES;
            segment[nsegments].x2 = lfoot[0][0];
            segment[nsegments].y2 = lfoot[0][1];
            segment[nsegments].concave = 0;
            nsegments++;
            for (i=0; i<NTREES-1; i++)
            {
            ...
            segment[nsegments].x1 = segment[nsegments-1].x2;
            segment[nsegments].y1 = segment[nsegments-1].y2;
            segment[nsegments].x2 = 0.5*(XMAX + segment[nsegments-1].x2);
            segment[nsegments].y2 = YMIN + OFSSET_TREES;
            segment[nsegments].concave = 0;
            nsegments++;
            segment[nsegments].x1 = segment[nsegments-1].x2;
            segment[nsegments].y1 = segment[nsegments-1].y2;
            segment[nsegments].x2 = XMAX;
            segment[nsegments].y2 = YMIN + OFSSET_TREES;
            segment[nsegments].concave = 0;
            nsegments++;
            ...
            for (i=0; i<nsegments; i++) 
                fprintf(lj_log, "Segment %i, (%.3lg, %.3lg) -- (%.3lg, %.3lg)\n", i, segment[i].x1, segment[i].y1, segment[i].x2, segment[i].y2);
...
{
    double x1, y1, x2, y2, r, f, angle, aniso, fx, fy, ff[2], dist_scaled, spin_f, ck, sk, ck_rel, sk_rel, alpha, amp, charge, f1, ffm[3], distance1, cos_sin[2];
...
//             distance1 = distance - 0.5*particle[i].radius;
            f = -KCOULOMB_FACTOR*krepel*charge/(1.0e-12 + distance*distance);
//             if (charge > 0.0) printf("Distance = %.3lg, force = %.3lg\n", distance, f);
            if (charge <= 0.0) 
...
//             force[1] += f*sa; 
        ...
        case (I_LJ_SPHERE):     /* redundant with I_LENNARD_JONES */
        {
            f = krepel*lennard_jones_force(distance, particle[k]);
            force[0] = f*ca;
            force[1] = f*sa;   
...
//                 printf("force = (%.3lg, %.3lg)\n", ff[0], ff[1]);
            ...
            default: 
            {
                ...
                if (SPHERE)
                {
                    angle = argument(ca_rel, sa_rel);
                    *torque = sin(spin_f*(angle - particle[i].angle))/(1.0e-8 + dist_scaled*dist_scaled);
                }
                else
                {
...
//     if (BOUNDARY_COND == BC_SPHERE) return(1);
...
/* returns 1 if distance between particles is smaller than NBH_DIST_FACTOR*MU */
{
    double distance, ca, sa, cj, sj, ca_rel, sa_rel, f[2], ff[2], torque1, ck, sk, ck_rel, sk_rel, caprime, saprime, cda, sda;
    ...
    if (BOUNDARY_COND == BC_SPHERE) distance = distance_sphere(i, k, particle, &ca, &sa);
    else distance = module2(particle[i].deltax[j], particle[i].deltay[j]);
...
//     }
    ...
    else
    {
        ...
        if (ROTATION)
        {
            ...
            {
                /* beta prime */
                cj = cos(particle[k].angle);
                sj = sin(particle[k].angle);
                distance = distance_sphere(k, i, particle, &caprime, &saprime);
                /* alpha prime - alpha - pi */
                cda = -ca*caprime - sa*saprime;
                sda = sa*caprime - ca*saprime;
                /* alpha prime - alpha - pi + beta prime*/
                ca_rel = cda*cj - sda*sj;
                sa_rel = sda*cj + cda*sj;
            }
            else
            {
                /* should this be particle k? */
...
/* add new particles in list nlist to old particle i, for paired particles */
{
    ...
    printf("Pairing particle %i, pairing type %i, npartners = %i\n", i, pairing_type, npartners);
    
    fprintf(lj_log, "Pairing particle %i, pairing type %i, npartners = %i\n", i, pairing_type, npartners);
    fprintf(lj_log, "newr = %.3lg, newcharge = %.3lg\n", newr, newcharge);
//     sleep(2);
...
//         printf("Particle %i mol_angle = %i\n", i, particle[k].mol_angle);
        ...
        switch (pairing_type) {
            ...
            case (POLY_PLUSMINUS):
            {
                ...
                {
//                     particle[n].charge = oldcharge;
                    if (ALTERNATE_POLY_CHARGE) particle[n].charge = newcharge;
                    else particle[n].charge = oldcharge;
                    particle[n].type = newtype;
                }
                else 
                {
...
/* initialize data in molecule structure */
{
    int i, m, np;
...
{
    int i, k, l, n, p, q, counter, nlist[NMAXPARTNERS], rtype = 0, newrtype, npartners;
    double angle, dist;
    ...
    printf("Pairing particles, pairing type %i\n", PAIRING_TYPE);
    
    nmolecules = ncircles;
    
    if (PAIRING_TYPE == POLY_NONE) npartners = 0;
    else npartners = NPARTNERS;
...
//                     printf("new type = %i\n", newrtype); 
    ...
    if (PAIRING_TYPE != POLY_NONE) ncircles += ncircles*npartners;
...
int add_particle(double x, double y, double vx, double vy, double mass, short int type, t_particle particle[NMAXCIRCLES], short int test, int time)
{
    int i, k, l, n, closeby = 0, nlist[NMAXPARTNERS];
    double dist, angle;
    
    /* test distance to other particles */
    if (test) for (i=0; i<ncircles; i++)
    {
        dist = module2(x - particle[i].xc, y - particle[i].yc);
        if ((particle[i].active)&&(dist < SAFETY_FACTOR*particle[i].radius)) closeby = 1;
...
//         particle[i].radius = MU*sqrt(mass);
        ...
        particle[i].thermostat = THERMOSTAT;
        particle[i].mass_inv = 1.0/mass;
        particle[i].charge = CHARGE_ADD;
        if ((ADD_ALTERNATE_CHARGE)&&((double)rand()/RAND_MAX <= ALTERNATE_CHARGE_PROPORTION)) 
        {
            particle[i].charge = CHARGE_ADD_B;
            particle[i].type++;
            particle[i].radius = MU_ADD_B;
            particle[i].mass_inv = 1.0/PARTICLE_ADD_MASS_B;
        }
        if ((TIME_DEPENDENT_ADD_CHARGE)&&((double)time/(NSTEPS - FINAL_NOADD_PERIOD) >= 1.0 - ALTERNATE_CHARGE_PROPORTION)) 
        {
            particle[i].charge = CHARGE_ADD_B;
            particle[i].type++;
            particle[i].radius = MU_ADD_B;
...
//         }
        ...
        fprintf(lj_log, "Time %i, Added particle at (%.3lg, %.3lg)\n", time, x, y);
        fprintf(lj_log, "Number of particles: %i\n", ncircles);
...
double compute_current(t_particle particle[NMAXCIRCLES])
{
    int i, nactive = 0;
    double current = 0.0;
    ...
    {
        current += particle[i].charge*particle[i].vx;
        nactive++;
    }
    return(current/(double)nactive); 
}
...
//     printf("Number of nbs: ");
    for (i=0; i<ncircles; i++) if (particle[i].active)
    {
        ...
        if (nsame > 1)
        {
            ...
            else for (k=0; k<3; k++) rgb[k] = particle[i].rgb[k];
...
//             if (x2 > x1 + PI) x2 -= PI;
//             if (x2 < x1 - PI) x2 += PI;
            length = dist_sphere(x1, y1, x2, y2)/particle.radius;
        }
        else 
...
//             if (length < 0.25*NBH_DIST_FACTOR) linkcolor = 1.0;
//             else linkcolor = 1.0 - 0.75*(length/NBH_DIST_FACTOR - 0.25);
            linkcolor = 0.1*(1.0 + 9.0*exp(-2.0*length/BOND_DIST_FACTOR));
            glColor3f(linkcolor, linkcolor, linkcolor);
        }
                
        if (length < 0.7*BOND_DIST_FACTOR) 
...
{
    double x1, y1, x2, y2, omega, r, rfactor;
    int wsign, i, j;
    ...
    switch(RD_REACTION)
    {
        ...
        case (CHEM_ABC):
        {
            if (particle.type == 3) 
            {
                ...
                for (wsign = -1; wsign <= 1; wsign+=2)
                {
                    x1 = xc1 + (double)wsign*0.5*radius*cos(particle.angle);
                    y1 = yc1 + (double)wsign*0.5*radius*sin(particle.angle);
                    if (wsign == 1) 
                    {
                        if (fill) draw_colored_polygon(x1, y1, rfactor*MU_B, nsides, angle + APOLY*PID, rgb);
                        else draw_polygon(x1, y1, rfactor*MU_B, nsides, angle + APOLY*PID);
                    }
                    else 
                    {
                        if (fill) draw_colored_polygon(x1, y1, rfactor*MU, nsides, angle + APOLY*PID, rgb);
                        else draw_polygon(x1, y1, rfactor*MU, nsides, angle + APOLY*PID);
                    }
                }
                return(0);
            }
        ...
        case (CHEM_AABAA):
        {
            if (particle.type == 2) 
            {
                for (wsign = -1; wsign <= 1; wsign+=2)
                {
                    x1 = xc1 + (double)wsign*0.55*radius*cos(particle.angle);
                    y1 = yc1 + (double)wsign*0.55*radius*sin(particle.angle);
                    if (fill) draw_colored_polygon(x1, y1, 0.7*radius, nsides, angle + APOLY*PID, rgb);
                    else draw_polygon(x1, y1, 0.7*radius, nsides, angle + APOLY*PID);
                }
                return(0);
...
void draw_collisions(t_collision *collisions, int ncollisions, t_hashgrid hashgrid[HASHX*HASHY])
/* draw discs where collisions happen */
{
    int i, j, cell;
    double rgb[3], rgb_bg[3], lum, lum1, x, y, x1, y1;
    ...
    for (i=0; i<ncollisions; i++) if (collisions[i].time > 0)
    {
        ...
        if (COLOR_BACKGROUND)
        {
            ...
            if (collisions[i].color == 0.0) for (j=0; j<3; j++) rgb[j] = lum1*rgb_bg[j] + lum;
            else 
            {
                hsl_to_rgb_palette(collisions[i].color, 0.9, lum, rgb, COLOR_PALETTE);
                for (j=0; j<3; j++) rgb[j] += lum1*rgb_bg[j];
            }
        }
        else
        {
...
//         printf("collision %i rgb = (%.3lf, %.3lf, %.3lf)\n", i, rgb[0], rgb[1], rgb[2]);
...
void draw_trajectory(t_tracer trajectory[TRAJECTORY_LENGTH*N_TRACER_PARTICLES], t_hashgrid hashgrid[HASHX*HASHY], int traj_position, int traj_length, t_particle *particle, t_cluster *cluster, int *tracer_n, int plot)
/* draw tracer particle trajectory */
{
    int i, j, time, p, width, imin, k, cell;
...
//     fprintf(lj_log, "imin = %i, traj_position = %i, traj_length = %i\n", imin, traj_position, traj_length); 
...
//                 printf("Drawing tracer %i\n", j);
                ...
                {
                    lum1 = 0.5*(1.0-lum);
                    glColor3f(lum1 + lum*particle[tracer_n[j]].rgb[0], lum1 + lum*particle[tracer_n[j]].rgb[1], lum1 + lum*particle[tracer_n[j]].rgb[2]);
                }
                else if (COLOR_BACKGROUND)
                {
                    ...
                    rgb_bg[1] = hashgrid[cell].g;
                    rgb_bg[2] = hashgrid[cell].b;
                    glColor3f(lum1*rgb_bg[0] + lum*particle[tracer_n[j]].rgb[0], lum1*rgb_bg[1] + lum*particle[tracer_n[j]].rgb[1], lum1*rgb_bg[2] + lum*particle[tracer_n[j]].rgb[2]);
                }
                else glColor3f(lum*particle[tracer_n[j]].rgb[0], lum*particle[tracer_n[j]].rgb[1], lum*particle[tracer_n[j]].rgb[2]);
...
//     blank();
    ...
    for (j=0; j<n_tracers; j++)
    {        
        compute_particle_colors(particle[tracer_n[j]], cluster, plot, rgb, rgbx, rgby, particle);
...
/* OBSOLETE - replaced by compute_background_color and draw_background */
{
...
void draw_background(t_particle particle[NMAXCIRCLES], t_segment segment[NMAXSEGMENTS], t_obstacle obstacle[NMAXOBSTACLES], int bg_color, t_hashgrid hashgrid[HASHX*HASHY])
/* color background according to particle properties */
{
    int i, j, k, n, p, q, m, nnb, number, avrg_fact, obs;
    double rgb[3], hue, value, p1, p2, pp1, pp2, oldhue, valx, valy, lum;
...
//     else 
    if ((!TRACER_PARTICLE)&&(!COLOR_BACKGROUND)&&(!DRAW_OBSTACLE_LINKS)&&(!FILL_OBSTACLE_TRIANGLES)&&(!REACTION_DIFFUSION)) blank();
...
//     if ((REACTION_DIFFUSION)&&(ncollisions > 0)) draw_collisions(collisions, ncollisions);
    ...
    {
//         compute_particle_colors(particle[j], cluster, plot, rgb, rgbx, rgby, particle);
        ...
        if (BOUNDARY_COND == BC_SPHERE)
        {
            ...
            for (i=-1; i<2; i++)
                draw_one_particle(particle[j], x1 + (double)i*DPI, y1, radius, angle, nsides, width, particle[j].rgb);
        }
        else if ((PERIODIC_BC)&&(plot != P_NOPARTICLE))
        {
            x1 = particle[j].xc;
            y1 = particle[j].yc;
            ...
            for (i=-1; i<2; i++)
                for (k=-1; k<2; k++)
                    draw_one_particle(particle[j], x1 + (double)i*(BCXMAX - BCXMIN), y1 + (double)k*(BCYMAX - BCYMIN), radius, angle, nsides, width, particle[j].rgb);
        }
            ...
            for (i=-2; i<3; i++)
            {
                ...
                for (k=-1; k<2; k++)
                    draw_one_particle(particle[j], x1 + (double)i*(BCXMAX - BCXMIN), sign*(y1 + (double)k*(BCYMAX - BCYMIN)), 
                                      radius, angle1, nsides, width, particle[j].rgb);
            }
        }
        else if ((BOUNDARY_COND == BC_BOY)&&(plot != P_NOPARTICLE))
        {
            ...
            for (i=-1; i<2; i++) for (k=-1; k<2; k++)
            {
                ...
                if (sign == -1.0) draw_one_particle(particle[j], signy*(x1 + (double)i*(BCXMAX - BCXMIN)), 
                    sign*(y1 + (double)k*(BCYMAX - BCYMIN)), radius, angle1, nsides, width, particle[j].rgbx);
                else if (signy == -1.0) draw_one_particle(particle[j], signy*(x1 + (double)i*(BCXMAX - BCXMIN)), 
                    sign*(y1 + (double)k*(BCYMAX - BCYMIN)), radius, angle1, nsides, width, particle[j].rgby);
                else draw_one_particle(particle[j], signy*(x1 + (double)i*(BCXMAX - BCXMIN)), 
                    sign*(y1 + (double)k*(BCYMAX - BCYMIN)), radius, angle1, nsides, width, particle[j].rgb);
            }
        }
        else if ((BOUNDARY_COND == BC_GENUS_TWO)&&(plot != P_NOPARTICLE))
        {
            ...
            if ((x1 < 0.0)&&(y1 < 0.0))
                for (i=-1; i<2; i++)
                    for (k=-1; k<2; k++)
                    {
                        ...
                        draw_one_particle(particle[j], x, y, radius, angle, nsides, width, particle[j].rgb);
                    }
            ...
            draw_one_particle(particle[j], particle[j].xc, particle[j].yc, radius, angle, nsides, width, particle[j].rgb);
...
void draw_absorbers(t_absorber absorber[NMAX_ABSORBERS])
/* draw the absorbing discs */
{
    int i, p;
    double rgb[3], s;
    ...
    for (i=0; i<nabsorbers; i++)
        for (p=-1; p<2; p++)
        {
            s = sin(absorber[i].yc);
            if (s > 0.1)
                draw_colored_ellipse_precomp(absorber[i].xc + p*DPI, absorber[i].yc, absorber[i].radius/s, absorber[i].radius, rgb);
            else
            {
                if (absorber[i].yc > 0.0)
                    draw_colored_rectangle(XMIN, YMAX - absorber[i].radius, XMAX, YMAX, rgb);
                else
                    draw_colored_rectangle(XMIN, YMIN, XMAX, YMIN + absorber[i].radius, rgb);
            }
        }
} 
...
//     fprintf(lj_log, "Facet %i area %.3lg, facet area0 %.3lg\n", nfacet, area, facet[nfacet].area0);
    
    area = 0.6 + OBSTACLE_AREA_SHADE_FACTOR*(area - facet[nfacet].area0);
    if (area > 1.0) area = 1.0;
    if (area < 0.0) area = 0.0;
    for (k=0; k<3; k++) rgb[k] = area;
}
...
/* draw the triangles between obstacles, for option FILL_OBSTACLE_TRIANGLES */
{
    ...
    short int acute;
    double rgb[3], pvect, shiftx, shifty, deltax2, deltay2, deltax3, deltay3;
    static double dx, dy;
    static int first = 1;
    ...
    if (FILL_OBSTACLE_TRIANGLES) 
    {
        ...
        for (t = 0; t < n_otriangles; t++)
        {
            ...
            deltax2 = -dx*(double)otriangle[t].shiftx[1];
            deltay2 = -dy*(double)otriangle[t].shifty[1];
            deltax3 = -dx*(double)otriangle[t].shiftx[2];
            deltay3 = -dy*(double)otriangle[t].shifty[2];
            
            pvect = (obstacle[s2].xc - obstacle[s1].xc + deltax2)*(obstacle[s3].yc - obstacle[s1].yc + deltay3);
            pvect -= (obstacle[s2].yc - obstacle[s1].yc + deltay2)*(obstacle[s3].xc - obstacle[s1].xc + deltax3);
        ...
        if (SHADE_OBSTACLE_FACETS) for (i = 0; i < n_ofacets; i++) 
        {
            ...
            for (f = 0; f < nf; f++) 
            {
                ...
                if (otriangle[t].acute)
                {
                    ...
                    deltax2 = -dx*(int)otriangle[t].shiftx[1];
                    deltay2 = -dy*(int)otriangle[t].shifty[1];
                    deltax3 = -dx*(int)otriangle[t].shiftx[2];
                    deltay3 = -dy*(int)otriangle[t].shifty[2];
        
                    draw_colored_triangle(obstacle[s1].xc, obstacle[s1].yc, obstacle[s2].xc + deltax2, obstacle[s2].yc + deltay2, obstacle[s3].xc + deltax3, obstacle[s3].yc + deltay3, rgb);
                    
                    /* TODO: fix boundary conditions */
                    draw_colored_triangle(obstacle[s1].xc + dx, obstacle[s1].yc, obstacle[s2].xc + deltax2 + dx, obstacle[s2].yc + deltay2, obstacle[s3].xc + deltax3 + dx, obstacle[s3].yc + deltay3, rgb);
                }
            }
        }
...
//             draw_colored_triangle(obstacle[s1].xc, obstacle[s1].yc, obstacle[s2].xc, obstacle[s2].yc, obstacle[s3].xc, obstacle[s3].yc, rgb);
    ...
    for (i = 0; i < nobstacles; i++)
        for (j = 0; j < obstacle[i].nneighb; j++)
        {
            ...
            draw_line(obstacle[i].xc, obstacle[i].yc, obstacle[neigh].xc + shiftx, obstacle[neigh].yc + shifty);
            
//             fprintf(lj_log, "Obstacle %i, neighbour %i = obs %i, shiftx = %i, shifty = %i\n", i, j, neigh, obstacle[i].shiftx[j], obstacle[i].shifty[j]);
//             fprintf(lj_log, "Drawing line from obs %i to obs %i: (%.3lg, %.3lg) - (%.3lg, %.3lg)\n", i, neigh, obstacle[i].xc, obstacle[i].yc, obstacle[neigh].xc + shiftx, obstacle[neigh].yc + shifty);  
        }
}
...
//     for (j=0; j<N_PRESSURES; j++) printf("Mean pressure[%i] = %.5lg\n", j, meanpress[j]);
    ...
    if (DRAW_SPHERE) y = 1.0 - 0.1*scale;
    else y = YMAX - 0.1*scale;
    ...
    if (PRINT_NABSORBED)
    {
        ...
        glColor3f(1.0, 1.0, 1.0);
        sprintf(message, "%i in pocket", params.nabsorbed);
    ...
    else if ((INCREASE_E)||(OSCILLATE_E))  /* print electric field */
    {
        ...
        if (PRINT_CURRENT)
        {
            ...
            erase_area_hsl(xbox, y + 0.025*scale, 0.27*scale, 0.05*scale, 0.0, 0.9, 0.0);
            glColor3f(1.0, 1.0, 1.0);
            sprintf(message, "Current = %.2f", params.current);
            write_text(xtext + 0.08, y, message);
        }
        ...
        {
            glLineWidth(2.0);
            if (params.efield > 0.0)
                draw_arrow(0.5*(XMIN+XMAX), YMAX - E_ARROW_YSHIFT, 0.5*(XMIN+XMAX) + params.efield/EFIELD + 0.05, YMAX - E_ARROW_YSHIFT, 15.0, 0.05);
            else
                draw_arrow(0.5*(XMIN+XMAX), YMAX - E_ARROW_YSHIFT, 0.5*(XMIN+XMAX) + params.efield/EFIELD - 0.05, YMAX - E_ARROW_YSHIFT, 15.0, 0.05);
        }
        
        if (DRAW_CURRENT_ARROW)
        {
            glLineWidth(2.0);
            if (params.efield > 0.0)
                draw_arrow(0.5*(XMIN+XMAX), YMIN + E_ARROW_YSHIFT, 0.5*(XMIN+XMAX) + params.current/MAX_CURRENT + 0.05, YMIN + E_ARROW_YSHIFT, 15.0, 0.05);
            else
                draw_arrow(0.5*(XMIN+XMAX), YMIN + E_ARROW_YSHIFT, 0.5*(XMIN+XMAX) + params.current/MAX_CURRENT - 0.05, YMIN + E_ARROW_YSHIFT, 15.0, 0.05);
        }
    }   
    ...
    if (PRINT_TYPE_PROP)   /* print proportion of types */
    {
        erase_area_hsl(xbox, y + 0.025*scale, 0.27*scale, 0.05*scale, 0.0, 0.9, 0.0);
        glColor3f(1.0, 1.0, 1.0);
        if (INTERACTION == I_COULOMB_IMAGINARY)
        {
            if (CHARGE > 0.0) sprintf(message, "%.1f %% rightons", 100.0*params.prop);
            else sprintf(message, "%.1f %% leftons", 100.0*params.prop);
        }
        else
        {
            if (CHARGE > 0.0) sprintf(message, "%.1f %% cations", 100.0*params.prop);
            else sprintf(message, "%.1f %% anions", 100.0*params.prop);
        }
        write_text(xtext + 0.08, y, message);
    } 
...
{
    int i, s, k, corner, group, ngroups, p, q;
    ...
    if (ADD_FIXED_OBSTACLES) for (i=0; i<nobstacles; i++)
    {
        if (PERIODIC_BC)
        {
            ...
            for (p=-1; p<2; p++)
                for (q=-1; q<2; q++)
                {
                    x1 = particle[j].xc - obstacle[i].xc + (XMAX-XMIN)*(double)p;
                    y1 = particle[j].yc - obstacle[i].yc + (YMAX-YMIN)*(double)q;
                    distance1 = module2(x1, y1);
...
/* compute force of partner particle n on particle j */
{
    ...
    double x1, x2, y1, y2, rmax, alpha, cosa, rj, rn, phi, dvx, dvy, pscal, newdistance, xg, yg, vxg, vyg;
    int p, q;
...
//     else
    {
        if (BOUNDARY_COND == BC_SPHERE) 
        {
            r = distance_sphere(j, n, particle, &ca, &sa);
...
//         if (r < 2.0*eq_distance) force = KSPRING_PAIRS*(r - eq_distance);
//         else 
//         {
//         printf("Dissociating partners %i and %i because max distance exceeded\n", j, n);
//             force = 0.0;
            /* TEST */
//             force = KSPRING_PAIRS*eq_distance;
//         dissociate_particles_findp(j, n, particle);
//         dissociate_molecule(j, n, particle);
//         }
        force = KSPRING_PAIRS*(r - eq_distance);
        
        if (DAMPING_PAIRS > 0.0)
        {
            dvx = particle[n].vx - particle[j].vx;
            dvy = particle[n].vy - particle[j].vy;
            pscal = dvx*ca + dvy*sa; 
            force -= pscal*DAMPING_PAIRS;
        }
        ...
        /* TEST - experimental */
        if (CORRECT_EQUILIBRIUM_POSITION)
        {
            newdistance = (1.0 - NUDGE_FACTOR)*r + NUDGE_FACTOR*eq_distance;
            if (BOUNDARY_COND == BC_SPHERE)
            {
//                 printf("phi1 = %.3lg, theta1 = %.3lg\n", particle[j].xc, particle[j].yc);
//                 printf("phi2 = %.3lg, theta2 = %.3lg\n", particle[n].xc, particle[n].yc);
                
                compute_convex_combination_sphere(particle[j].xc, particle[j].yc, particle[n].xc, particle[n].yc, &particle[j].xc, &particle[j].yc, &particle[n].xc, &particle[n].yc, newdistance/r);
//                 particle[j].xc = xg;
//                 particle[j].yc = yg;
//                 printf("xg = %.3lg, yg = %.3lg\n", xg, yg);
            }
            else
            {
                xg = 0.5*(particle[n].xc + particle[j].xc);
                yg = 0.5*(particle[n].yc + particle[j].yc);
                particle[n].xc = xg + 0.5*ca*newdistance;
                particle[n].yc = yg + 0.5*sa*newdistance;
                particle[j].xc = xg - 0.5*ca*newdistance;
                particle[j].yc = yg - 0.5*sa*newdistance;
            }
//             vxg = 0.5*(particle[j].vx + particle[n].vx);
//             vyg = 0.5*(particle[j].vy + particle[n].vy);
//             particle[j].vx = vxg;
//             particle[j].vy = vyg;
//             particle[n].vx = vxg;
//             particle[n].vy = vyg;
        }
//         f[0] -= particle[j].vx*DAMPING_PAIRS;
//         f[1] -= particle[j].vy*DAMPING_PAIRS;
        
//         particle[j].vx *= exp(-DAMPING_PAIRS);
//         particle[j].vy *= exp(-DAMPING_PAIRS);
//         particle[n].vx *= exp(-DAMPING_PAIRS);
//         particle[n].vy *= exp(-DAMPING_PAIRS);
...
//         if ((PLOT == P_CLUSTER)||(PLOT_B == P_CLUSTER))
    ...
    {   
        ...
        y = YMIN + 2.0*MU;
        add_particle(x, y, 0.0, 0.0, MASS_PART_BOTTOM, 0, particle, 1, 0);
    }
    if (PART_AT_BOTTOM) for (i=0; i<=NPART_BOTTOM; i++)
    {   
        x = XMIN + (double)i*(XMAX - XMIN)/(double)NPART_BOTTOM;
    ...
    /* TODO: different options for number of tracers */
    if (TRACER_PARTICLE) 
    {
        printf("Initializing tracers\n");
        i = 0;
        while ((n_tracers < N_TRACER_PARTICLES)&&(i < ncircles))
        {
...
//             if (particle[i].charge != 0.0) particle[i].radius = MU_C;
    ...
    /* WARNING: may interfere with PAIR_PARTICLES */
    if ((REACTION_DIFFUSION)||(SPECIAL_IC)) for (i=0; i< ncircles; i++)
    {
        switch (RD_INITIAL_COND) {
            ...
            case (IC_LAYERS):
            {
                if (particle[i].yc > (1.0 - TYPE_PROPORTION)*YMAX + TYPE_PROPORTION*YMIN + 0.5*LAMBDA)
                {
                    particle[i].type = 1;
                    particle[i].radius = MU;
                    particle[i].eq_dist = EQUILIBRIUM_DIST;
                ...
                else if (particle[i].yc < (1.0 - TYPE_PROPORTION)*YMAX + TYPE_PROPORTION*YMIN) 
                {
                    particle[i].type = 2;
                    particle[i].radius = MU_B;
                    particle[i].eq_dist = EQUILIBRIUM_DIST_B;
            ...
            case (IC_STRIPES):
            {
                dx = (XMAX - XMIN)/(double)NGRIDX;
                dy = (YMAX - YMIN)/(double)NGRIDY;
                xx = (particle[i].xc - XMIN)/dx;
                yy = (particle[i].yc - YMIN)/dy;
//                 if ((int)xx%2 == 1) yy -= 0.5*dy;
//                 printf("Stripes: y = %.3lg, yy = %.3lg, test = %i\n", particle[i].yc, yy, (int)yy%2); 
//                 if ((int)yy%2 == 0)
                if (i%2 == 0)
                {
                    particle[i].yc += 0.2*dy;
                    particle[i].type = 1;
                    particle[i].radius = MU;
                    particle[i].eq_dist = EQUILIBRIUM_DIST;
                    particle[i].mass_inv = 1.0/PARTICLE_MASS;
            ...
            case (IC_SIGNY_LAYER):
            {
                if (vabs(particle[i].yc) < 0.1*YMAX) particle[i].active = 0;
                else if (particle[i].yc < 0.0) 
                {
                    particle[i].type = 1;
                    particle[i].charge = CHARGE;
                }
...
int tracer_n[N_TRACER_PARTICLES], int time)
/* add several particles to the system */
{
    ...
    add_particle(x, y, V_INITIAL_ADD, 0.0, PARTICLE_ADD_MASS, type, particle, 0, time);
    
    printf("Particle number %i\n", ncircles-1);
    
//     add_particle(x, y, V_INITIAL_ADD, 0.05*V_INITIAL_ADD*(double)rand()/RAND_MAX, PARTICLE_ADD_MASS, type, particle, 0, time);
...
//             inv_masses[6] = 0.2/PARTICLE_MASS;
        ...
        case (CHEM_FISSION):
        {
            inv_masses[2] = inv_masses[1]/235.0;
            inv_masses[3] = inv_masses[1]/95.0;
            inv_masses[4] = inv_masses[1]/139.0;
            inv_masses[5] = inv_masses[1]/233.0;
            inv_masses[6] = inv_masses[1]/2.0;
            break;
        }
        case (CHEM_FISSION_CD):
        {
            mass = 1.0/PARTICLE_ADD_MASS;
            inv_masses[3] = mass/113.0;
            inv_masses[4] = mass;
            inv_masses[5] = mass/95.0;
            inv_masses[6] = mass/139.0;
            inv_masses[7] = mass/233.0;
            inv_masses[8] = mass/2.0;
            break;
        }
        case (CHEM_FISSION_ABS):
        {
            mass = 1.0/PARTICLE_ADD_MASS;
            inv_masses[3] = mass/113.0;
            inv_masses[4] = mass;
            inv_masses[5] = mass/95.0;
            inv_masses[6] = mass/139.0;
            inv_masses[7] = mass/140.0;
            inv_masses[8] = mass/233.0;
            inv_masses[9] = mass/2.0;
            break;
        }
    }
}
...
/* merging of particle i and a particle of type type2 into a particle of type newtype */
{
    int j, k, type1;
    short int search = 1;
    double distance, rx, ry, m2, mr1, mr2, newmass_inv, old_energy, e_ratio, dx, dy;
    ...
    for (j=0; j<particle[i].hash_nneighb; j++) 
    {
        ...
        if ((particle[k].active)&&(particle[k].type == type2))
        {
            if (BOUNDARY_COND == BC_SPHERE)
            {
                dx = particle[i].deltax[j];
                dy = particle[i].deltay[j];
                distance = dist_sphere(particle[i].xc, particle[i].yc, particle[i].xc + dx, particle[i].yc + dy);
            }
            else distance  = module2(particle[i].deltax[j], particle[i].deltay[j]);
            if ((distance < REACTION_DIST*MU)&&((double)rand()/RAND_MAX < REACTION_PROB))
            {
                ...
                particle[i].mass_inv = newmass_inv;
                particle[i].charge += particle[k].charge;
                ...
                fprintf(lj_log, "\n[chem_merge] Collision %i at (%.3lg, %.3lg)\n", ncollisions, particle[i].xc, particle[i].yc);
...
int chem_split(int i, int newtype1, int newtype2, t_particle particle[NMAXCIRCLES], t_collision *collisions, int ncollisions, double inv_masses[RD_TYPES+1], double radii[RD_TYPES+1], int tracer_n[N_TRACER_PARTICLES] )
/* split particle i into particles of type newtype1 and newtype2 */
{
    ...
    rx = -particle[i].radius*particle[i].vy/normv;
    ry = particle[i].radius*particle[i].vx/normv;
    ...
    success = add_particle(xg - rx, yg - ry, particle[i].vx, particle[i].vy, 1.0/inv_masses[newtype1], newtype1, particle, 0, 0);
    if (success)
    {
        ...
        fprintf(lj_log, "\n[chem_split] Collision %i at (%.3lg, %.3lg)\n", ncollisions, particle[i].xc, particle[i].yc);
...
/* reaction of particle i and a particle of type type2 into a particles of types newtype1 and newtype2 */
{
    int j, k, type1;
    short int search = 1;
    double distance, rx, ry, mass_inv1, mass_inv2, newmass_inv1, newmass_inv2, old_energy, e_ratio, omega, xk, yk, dx, dy;
    ...
    for (j=0; j<particle[i].hash_nneighb; j++) 
    {
        ...
        if ((particle[k].active)&&(particle[k].type == type2))
        {
            ...
            if ((distance < reac_dist*MU)&&((double)rand()/RAND_MAX < REACTION_PROB))
            {
                ...
                rx = -particle[i].deltax[j];
                ry = -particle[i].deltax[j];
//                 rx = particle[i].xc - particle[k].xc;
//                 ry = particle[i].yc - particle[k].yc;
                particle[i].angle = argument(rx, ry);
...
//                 collisions[ncollisions].y = 0.5*(particle[i].yc + particle[k].yc);
                collisions[ncollisions].x = particle[i].xc - 0.5*rx;
                collisions[ncollisions].y = particle[i].yc - 0.5*ry;
                collisions[ncollisions].time = COLLISION_TIME;
                collisions[ncollisions].color = 0.0;
                
                if (vabs(particle[i].xc) < 0.25*XMAX)
                    fprintf(lj_log, "\n[chem_transfer] Collision %i at (%.3lg, %.3lg), delta = (%.3lg, %.3lg)\n", ncollisions, particle[i].xc, particle[i].yc, rx, ry);
...
/* having at most nmaxpartners partners already */
{
    ...
    while ((p<particle[i].hash_nneighb)&&(!reaction))
    {
        ...
        if ((particle[k].active)&&(particle[k].type == type2)&&(particle[k].npartners <= maxpartners))
        {
            ...
            {
                reaction = 1;
                printf("[chem_merge_molecule] Merging molecule %i with particle %i\n", i, k);
...
int chem_multi_glue_molecule(int i, int type2, int maxpartners, int no_triangles, int no_cluster, int require_charge, int equalize_charge, t_particle particle[NMAXCIRCLES], t_molecule molecule[NMAXCIRCLES], t_cluster cluster[NMAXCIRCLES], t_collision *collisions, int ncollisions, double reaction_prob, int mergeclusters)
...
{
    int k, p, q, p1, q1, p2, np, nq, n, m, closeby = 0, reaction = 0, jp[NMAXPARTNERS], jq[NMAXPARTNERS], r, kk, different, np2, moli, molk, mp, newpartner, pp, type1, pmi, pmk;
    double distance, angle, move_factor, xnew, ynew, vxnew, vynew, m1, m2, mr1, mr2, deltav, x, y, dx, dy; 
    short int charge_condition, triangle_condition; 
    
    type1 = particle[i].type;
    np = particle[i].npartners;
    moli = particle[i].molecule;
    pmi = molecule[moli].npartners;
    if (np > maxpartners) return(ncollisions);
    
//     fprintf(lj_log, "[chem_multi_glue_molecule] np = %i, pmi = %i\n", np, pmi); 
    
    /* allow only diatomic molecules to form */
    if ((no_triangles == -1)&&(pmi > 0)) return(ncollisions);
...
//         printf("[chem_multi_glue_molecule] nq = %i\n", nq); 
//         fprintf(lj_log, "[chem_multi_glue_molecule] nq = %i\n", nq); 
        
        molk = particle[k].molecule;
        pmk = molecule[molk].npartners;
...
//         fprintf(lj_log, "[chem_multi_glue_molecule] triangle_condition = %i\n", triangle_condition); 
...
//             if (molk == moli) triangle_condition = 0;
        ...
        /* also limit number of partners of second particle */
        if (no_triangles == -2)
        {
            if (nq > maxpartners + 1) triangle_condition = 0;
            if (moli == molk) triangle_condition = 0;
            for (q=0; q<nq; q++)
                for (p1=0; p1<np; p1++)
            ...
            fprintf(lj_log, "np = %i, nq = %i, trangle_condition = %i\n", np, nq, triangle_condition); 
        }
        ...
        /* Tallow only diatomic molecules to form */
        if ((no_triangles == -1)&&(pmk > 0)) triangle_condition = 0;
        
//         fprintf(lj_log, "[chem_multi_glue_molecule] triangle_condition = %i\n", triangle_condition); 
// //         fprintf(lj_log, "[chem_multi_glue_molecule] particle[k].active = %i\n", particle[k].active); 
//         fprintf(lj_log, "[chem_multi_glue_molecule] charge_condition = %i\n", charge_condition); 
//         fprintf(lj_log, "[chem_multi_glue_molecule] required type2 = %i\n", type2); 
//         fprintf(lj_log, "[chem_multi_glue_molecule] type2 = %i\n", particle[k].type); 
//         fprintf(lj_log, "[chem_multi_glue_molecule] (nq <= maxpartners) = %i\n", nq <= maxpartners); 
        
        fprintf(lj_log, "[Repeat] np = %i, nq = %i, trangle_condition = %i\n", np, nq, triangle_condition); 
        fprintf(lj_log, "active = %i, charge_condition = %i, triangle_condition = %i, type = %i, type2 = %i\n", particle[k].active, charge_condition, triangle_condition, particle[k].type, type2);
                
        if ((particle[k].active)&&(charge_condition)&&(triangle_condition)&&(particle[k].type == type2)&&(nq <= maxpartners))
        {
            if (BOUNDARY_COND == BC_SPHERE)
            {
                dx = particle[i].deltax[p];
                dy = particle[i].deltay[p];
                distance = dist_sphere(particle[i].xc, particle[i].yc, particle[i].xc + dx, particle[i].yc + dy);
            }
            else distance  = module2(particle[i].deltax[p], particle[i].deltay[p]);
            
//             fprintf(lj_log, "[chem_multi_glue_molecule] type1 = %i\n", type1); 
//             fprintf(lj_log, "[chem_multi_glue_molecule] type2 = %i\n", particle[k].type); 
//             fprintf(lj_log, "[chem_multi_glue_molecule] required type2 = %i\n", type2); 
//             fprintf(lj_log, "[chem_multi_glue_molecule] triangle_condition = %i\n", triangle_condition); 
//             fprintf(lj_log, "[chem_multi_glue_molecule] charge_condition = %i\n", charge_condition); 
//         
//             fprintf(lj_log, "[chem_multi_glue_molecule] distance = %.3lg\n", distance); 
//             fprintf(lj_log, "[chem_multi_glue_molecule] REACTION_DIST*MU = %.3lg\n", REACTION_DIST*MU); 
            
            if ((distance < REACTION_DIST*MU)&&((double)rand()/RAND_MAX < reaction_prob))
            {
//                 printf("[chem_multi_glue_molecule] np = %i\n", np); 
//                 printf("[chem_multi_glue_molecule] nq = %i\n", nq); 
//                 printf("[chem_multi_glue_molecule] triangle_condition = %i\n", triangle_condition); 
//                 printf("[chem_multi_glue_molecule] type1 = %i\n", type1); 
//                 printf("[chem_multi_glue_molecule] type2 = %i\n", particle[k].type); 
                
                fprintf(lj_log, "Collision %i\n", ncollisions); 
                fprintf(lj_log, "[chem_multi_glue_molecule] np = %i\n", np); 
                fprintf(lj_log, "[chem_multi_glue_molecule] nq = %i\n", nq); 
                fprintf(lj_log, "[chem_multi_glue_molecule] triangle_condition = %i\n", triangle_condition); 
                fprintf(lj_log, "[chem_multi_glue_molecule] type1 = %i\n", type1); 
                fprintf(lj_log, "[chem_multi_glue_molecule] type2 = %i\n", particle[k].type); 
                
                fprintf(lj_log, "-->\nMerging particles %i (cluster %i) and %i (cluster %i)\n", i, particle[i].cluster, k, particle[k].cluster); 
//                 sleep(1);
...
//                 sleep(1);
                ...
                {
                    reaction = 1;
                    printf("[chem_multi_glue_molecule] Merging molecule %i with particle %i\n", i, k);
...
//                     particle[i].partner_eqd[0] = MU + MU_B;    
                    
                    particle[k].npartners++;
                    particle[k].partner[0] = i;
                    particle[k].partner_eqd[0] = distance;
//                     particle[k].partner_eqd[0] = MU + MU_B;
...
//                     printf("1\n"); 
                    ...
                    if (mergeclusters)
                    {
                        ...
                        fprintf(lj_log, "merged clusters %i and %i\n", particle[i].cluster, particle[k].cluster);
                        fprintf(lj_log, "cluster %i has %i particles\n", particle[i].cluster, cluster[particle[i].cluster].nparticles); 
                        for (p=0; p<cluster[particle[i].cluster].nparticles; p++)
                            fprintf(lj_log, "%i ", cluster[particle[i].cluster].particle[p]);
                        fprintf(lj_log, "\ncluster %i has %i particles\n", particle[k].cluster, cluster[particle[k].cluster].nparticles); 
                        for (p=0; p<cluster[particle[k].cluster].nparticles; p++)
                            fprintf(lj_log, "%i ", cluster[particle[k].cluster].particle[p]);
                        fprintf(lj_log, "\n\n");
                    }
...
//                     printf("2\n"); 
                    ...
                    if (NEUTRALIZE_REACTING_PARTICLES)
                    {
                        particle[i].charge = 0.0;
                        particle[k].charge = 0.0;
                    }
                    
                    /* for P_MOL_ANGLE color scheme */
                    if (particle[i].charge > 0.0)
                    {
                        particle[i].p0 = k;
                        particle[i].p1 = i;
                        particle[k].p0 = k;
...
//                     printf("molecule %i has %i partners\n", moli, mp); 
                    ...
                    if (mp < NMAXPARTNERMOLECULES)
                    {
                        ...
                        if (newpartner)
                        {
                            ...
                            fprintf(lj_log, "adding molecule %i as %ith partner of molecule %i\n", molk, mp, moli);
                            molecule[moli].partner[mp] = molk;
                            molecule[moli].connection_type[mp] = particle[i].type;
                            molecule[moli].npartners++;
                            
                            printf("Partners of molecule %i:", moli);
                            fprintf(lj_log, "Partners of molecule %i:", moli);
                            for (pp=0; pp<molecule[moli].npartners; pp++) 
                            {
                                printf("%i ", molecule[moli].partner[pp]);
                                fprintf(lj_log, "%i ", molecule[moli].partner[pp]);
                            }
                            printf("\n");
                            fprintf(lj_log, "\n\n");
                        }
...
//                     printf("4\n"); 
                    ...
                    if (mp < NMAXPARTNERMOLECULES)
                    {
                        ...
                        if (newpartner)
                        {
                            ...
                            printf("Partners of molecule %i:", molk);
                            fprintf(lj_log, "Partners of molecule %i:", molk);
                            for (pp=0; pp<molecule[molk].npartners; pp++) 
                            {
                                printf("%i ", molecule[molk].partner[pp]);
                                fprintf(lj_log, "%i ", molecule[molk].partner[pp]);
                            }
                            printf("\n");
                            fprintf(lj_log, "\n\n");
                        }
...
//                                 if (np2 > 1)
            ...
            else fprintf(lj_log, "No collision, distance = %.3lg\n", distance);
        }
        p++;
    }
    return(ncollisions);
...
void translate_and_rotate_cluster(int j, t_cluster cluster[NMAXCIRCLES], t_particle particle[NMAXCIRCLES], double dx, double dy, double angle, int update_particles, int verbose)
/* translate and rotate a cluster and all partcles it contains */
{
    ...
    if (verbose) 
    {
        fprintf(lj_log, "Cluster %i has %i particles: ", j, np);
        for (k=0; k<np; k++) fprintf(lj_log, "%i, ", cluster[j].particle[k]);
        fprintf(lj_log, "\nAdding %.5lg to yg = %.5lg -> ", dy, cluster[j].yg);
    }
    
    if (np == 1)
    {
        p = cluster[j].particle[0];
        if (verbose) fprintf(lj_log, "yc = %.5lg\n", particle[p].yc);
        if (update_particles) 
        {
            particle[p].xc += dx;
            particle[p].yc += dy;
...
//         cluster[j].xg = particle[p].xc;
//         cluster[j].yg = particle[p].yc;
        if (verbose) fprintf(lj_log, "[np=1] Adding %.5lg to yg = %.5lg -> ", dy, cluster[j].yg);
        cluster[j].xg += dx;
        cluster[j].yg += dy;
//         particle[p].xc = cluster[j].xg;
//         particle[p].yc = cluster[j].yg;
        if (verbose) fprintf(lj_log, " %.5lg\n", cluster[j].yg);
        cluster[j].angle = particle[p].angle;
    }
    else
    {
        if (verbose) fprintf(lj_log, "[np=%i] Adding %.5lg to yg = %.5lg -> ", np, dy, cluster[j].yg);
        ca = cos(angle);
        sa = sin(angle);
        if (update_particles) for (k=0; k<np; k++)
        {
            p = cluster[j].particle[k];
            x = particle[p].xc - cluster[j].xg;
            y = particle[p].yc - cluster[j].yg;
...
//     if (verbose) printf(" %.5lg\n", cluster[j].yg);
}

void translate_molecule(int j, t_molecule molecule[NMAXCIRCLES], t_particle particle[NMAXCIRCLES], double dx, double dy)
/* translate a cluster and all partcles it contains */
{
    int k, p, np;
        
    np = molecule[j].nparticles;
    
    if (np == 1)
    {
        p = molecule[j].particle[0];
        particle[p].xc += dx;
        particle[p].yc += dy;
    }
    else 
...
/* returns number of active clusters */
{
    ...
    fprintf(lj_log, "Initializing clusters\n\n");
...
int update_types(t_particle particle[NMAXCIRCLES], t_molecule molecule[NMAXCIRCLES], t_cluster cluster[NMAXCIRCLES], t_collision *collisions, int ncollisions, int *particle_numbers, int time, double *delta_e, int tracer_n[N_TRACER_PARTICLES])
        ...
        case (CHEM_BAA):
        {
            for (i=0; i<ncircles; i++) 
                if ((particle[i].active)&&(particle[i].type == 2)&&((double)rand()/RAND_MAX < DISSOCIATION_PROB))
                    ncollisions = chem_split(i, 1, 1, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
            printf("%i collisions\n", ncollisions);
            return(ncollisions);
        }
        case (CHEM_AABAA):
        ...
        case (CHEM_POLYMER_DISS):
        {
            for (i=0; i<ncircles; i++)  if (particle[i].active)
            {
                if (particle[i].type == 1) for (k=2; k<RD_TYPES; k++)
                    ...
                    ncollisions = chem_split(i, 1, particle[i].type - 1, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
            }
...
//                     printf("Splitting type %i into type %i and type %i\n", type, k, type - k);
                    ncollisions = chem_split(i, k, type - k, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                }
            }
        ...
        case (CHEM_ABCAB):
        {
            *delta_e = 0.0;
            for (i=0; i<ncircles; i++)
            {
                ...
                {
                    ncollisions = chem_split(i, 1, 2, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                    if (EXOTHERMIC) *delta_e -= (double)(ncollisions - oldncollisions)*DELTA_EKIN;
                }
            }
            printf("Delta_E = %.3lg\n", *delta_e);
        ...
        case (CHEM_ABCDABC):
        {
            *delta_e = 0.0;
            for (i=0; i<ncircles; i++)
            {
                ...
                {
                    ncollisions = chem_split(i, 1, 3, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                    if (EXOTHERMIC) *delta_e -= (double)(ncollisions - oldncollisions)*DELTA_EKIN;
                }
            }
            printf("Delta_E = %.3lg\n", *delta_e);
...
//                             ncollisions = chem_transfer(i, 6, 1, 1, particle, collisions, ncollisions, inv_masses, radii, REACTION_DIST);
                    ...
                    case (7):
                    {
                        ncollisions = chem_split(i, 3, 3, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                        break;
                    }
                    case (8):
                    {
                        ncollisions = chem_split(i, 5, 5, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                        break;
                    }
                }
            }
...
//                     if (EXOTHERMIC) *delta_e += (double)(ncollisions - oldncollisions)*DELTA_EKIN;
        ...
        case (CHEM_AGGREGATION):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                ...
                {
                    ncollisions = chem_multi_glue_molecule(i, k, AGREGMAX, 2, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
        ...
        case (CHEM_AGGREGATION_MAX):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
//                 if (particle[i].npartners < AGREGMAX) for (k=0; k<3; k++)
                {
                    ncollisions = chem_multi_glue_molecule(i, 1, AGREGMAX, -2, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
        ...
        case (CHEM_AGGREGATION_CHARGE):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                if (particle[i].npartners < AGREGMAX) for (k=0; k<5; k++)
                {
                    ncollisions = chem_multi_glue_molecule(i, k, AGREGMAX, 0, 0, 1, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
        ...
        case (CHEM_AGGREGATION_CHARGE_NOTRIANGLE):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                if (particle[i].npartners < AGREGMAX) for (k=0; k<5; k++)
                {
                    ncollisions = chem_multi_glue_molecule(i, k, AGREGMAX, -1, 0, 1, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
...
//             init_cluster_config(particle, cluster);
        ...
        case (CHEM_DNA):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                if (particle[i].npartners < AGREGMAX) switch (particle[i].type) 
                {
                    case (2): 
                    {
                        ncollisions = chem_multi_glue_molecule(i, 2, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                        break;
                    }
                    case (3): 
                    {
                        ncollisions = chem_multi_glue_molecule(i, 4, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                        break;
                    }
                    case (4): 
                    {
                        ncollisions = chem_multi_glue_molecule(i, 3, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                        break;
                    }
                    case (5): 
                    {
                        ncollisions = chem_multi_glue_molecule(i, 6, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                        break;
                    }
                    case (6): 
                    {
                        ncollisions = chem_multi_glue_molecule(i, 5, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                        break;
                    }
                }
        ...
        case (CHEM_DNA_ALT):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                if (particle[i].npartners < AGREGMAX) 
                {
                    ...
                    if (atype > 0)
                        ncollisions = chem_multi_glue_molecule(i, btype, AGREGMAX, 0, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
        ...
        case (CHEM_DNA_DSPLIT):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                if (particle[i].npartners < AGREGMAX) 
                {
                    ...
                    if (atype > 0)
                        ncollisions = chem_multi_glue_molecule(i, btype, AGREGMAX, 3, 0, 0, 0, particle, molecule, cluster, collisions, ncollisions, REACTION_PROB, 0);
                }
        ...
        case (CHEM_FISSION):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                ...
                switch (particle[i].type) {
                    case (2): /* U + n -> A + B */
                    {
                        ncollisions = chem_transfer(i, 1, 5, 6, particle, collisions, ncollisions, inv_masses, radii, REACTION_DIST);
                        break;
                    }
                    case (5): /* A -> Sr + Xe */
                    {
                        if ((double)rand()/RAND_MAX < DISSOCIATION_PROB)
                            ncollisions = chem_split(i, 3, 4, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                        break;
                    }
                    case (6): /* B -> n + n */ 
                    {
        ...
        case (CHEM_FISSION_CD):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                ...
                switch (particle[i].type) {
                    ...
                    case (2): /* Cd112 + n -> Cd113 */
                    {
                        ncollisions = chem_merge(i, 4, 3, particle, collisions, ncollisions, inv_masses, radii);
                        break;
                    }
                    case (7): /* A -> Sr + Xe */
                    {
                        if ((double)rand()/RAND_MAX < DISSOCIATION_PROB)
                            ncollisions = chem_split(i, 5, 6, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                        break;
                    }
                    case (8): /* B -> n + n */ 
                    {
                        if ((double)rand()/RAND_MAX < DISSOCIATION_PROB)
                            ncollisions = chem_split(i, 4, 4, particle, collisions, ncollisions, inv_masses, radii, tracer_n);
                        break;
                    }
                }
            }
        ...
        case (CHEM_FISSION_ABS):
        {
            for (i=0; i<ncircles; i++) if (particle[i].active)
            {
                ...
                switch (particle[i].type) {
                    case (1): /* U + n -> A + B */
                    {
                        ncollisions = chem_transfer(i, 4, 8, 9, particle, collisions, ncollisions, inv_masses, radii, REACTION_DIST);
                        break;
                    }
                    case (2): /* Cd112 + n -> Cd113 */
                    {
                    ...
                    case (6): /* Fictitious reaction Xe139 + n -> Xe140 */
                    {
                        if ((double)rand()/RAND_MAX < 0.7)
                            ncollisions = chem_merge(i, 4, 7, particle, collisions, ncollisions, inv_masses, radii);
                        break;
                    }
                    case (8): /* A -> Sr + Xe */
                    {
...
double plot_coord_sym(double x, double xmin, double xmax)
{
    return(xmin + 0.5*(x+1.0)*(xmax - xmin));
}
...
double efield_schedule(int i)
{
    ...
    {
        if (INCREASE_E)
            efield = EFIELD*(double)(i-INITIAL_TIME)*efactor;
        else if (OSCILLATE_E)
            efield = EFIELD*sin(DPI*(double)(i-INITIAL_TIME)/(double)E_PERIOD);
    }
//     printf("E = %.3lg\n", efield);
    return(efield);
}
...
/* draw plot of current vs E field */
{
    int j;
    char message[100];
    static double xmin, xmax, ymin, ymax, xmid, ymid, dx, dy, plotxmin, plotxmax, plotymin, plotymax, rgb[3];
    double pos[2], x1, y1, x2, y2, lum;
    static int first = 1, gshift = INITIAL_TIME + NSTEPS;
    ...
    glLineWidth(2);
    glColor3f(1.0, 1.0, 1.0);
    draw_line(xmin, ymid, xmax, ymid);
    draw_line(xmid, ymin, xmid, ymax); 
    draw_rectangle(xmin, ymin, xmax, ymax); 
    
    glColor3f(rgb[0], rgb[1], rgb[2]);
    
    x1 = plot_coord_sym(efield_schedule(0)/EFIELD, plotxmin, plotxmax);
    y1 = plot_coord_sym(currents[0]/MAX_CURRENT, plotymin, plotymax);
    for (j=0; j<i; j++)
    {
        lum = exp(-2.0*(double)(i-j)/(double)NSTEPS);
//         printf("i-j = %i, lum = %.3lg\n", j-i, lum); 
        glColor3f(rgb[0]*lum, rgb[1]*lum, rgb[2]*lum);
        
        x2 = plot_coord_sym(efield_schedule(j)/EFIELD, plotxmin, plotxmax);
        y2 = plot_coord_sym(currents[j]/MAX_CURRENT, plotymin, plotymax);
        
        draw_line(x1, y1, x2, y2);
        x1 = x2;
        y1 = y2;
    }
    draw_colored_circle_precomp(x2, y2, 0.01, rgb);
    
    glColor3f(1.0, 1.0, 1.0);
    sprintf(message, "E");
    write_text_fixedwidth(plotxmax - 0.005, ymid - 0.05, message);
    sprintf(message, "I");
    write_text_fixedwidth(xmid - 0.05, plotymax - 0.005, message);
}
...
/* draw plot of number of particles as a function of time */
{
    ...
    {                
        ...
        nmax = 0;
        for (j=1; j<RD_PLOT_TYPES+1; j++) 
            if (particle_numbers[RD_PLOT_TYPES+1+j] > nmax) 
                nmax = particle_numbers[RD_PLOT_TYPES+1+j];
    ...
    for (type=1; type<RD_PLOT_TYPES+1; type++)
    {
...
int wrap_particles_periodic_bc(t_particle particle[NMAXCIRCLES], t_molecule molecule[NMAXCIRCLES], t_cluster cluster[NMAXCIRCLES])
/* wrap particles for certain boundary conditions */
{
    int i, j, k, p, mol, clust, move = 0;
    short int *move_part, *move_molecule, *move_cluster, *molecule_refparticle, *cluster_refparticle;
    static double dx, dy, paddingx, paddingy;
    static int first = 1;
    double *deltax, *deltay;
    
    move_part = (short int *)malloc(ncircles*sizeof(short int));
    move_molecule = (short int *)malloc(ncircles*sizeof(short int));
    molecule_refparticle = (short int *)malloc(ncircles*sizeof(short int));
    cluster_refparticle = (short int *)malloc(ncircles*sizeof(short int));
    move_cluster = (short int *)malloc(ncircles*sizeof(short int));
    deltax = (double *)malloc(ncircles*sizeof(double));
    deltay = (double *)malloc(ncircles*sizeof(double));
    
    if (first)
    {
        dx = BCXMAX - BCXMIN;
        dy = BCYMAX - BCYMIN;
        paddingx = 0.5*dx/(double)HASHX;
        paddingy = 0.5*dy/(double)HASHY;
        first = 0;
    }
    
//     #pragma omp parallel for private(i)
    /* determine circles to be moved */
    for (i=0; i<ncircles; i++)
    {
        move_part[i] = 0;
        deltax[i] = 0.0;
        deltay[i] = 0.0;
        if (particle[i].xc > BCXMAX + paddingx) 
        {
            move_part[i] = 1;
            deltax[i] = -dx;
        }
        else if (particle[i].xc < BCXMIN - paddingx)
        {
            move_part[i] = 1;
            deltax[i] = dx;
        }
        if (particle[i].yc > BCYMAX + paddingy) 
        {
            move_part[i] = 1;
            deltay[i] = -dy;
        }
        else if (particle[i].yc < BCYMIN - paddingy)
        {
            move_part[i] = 1;
            deltay[i] = dy;
        }
        move += move_part[i];
//         if (move_part[i]) 
//             fprintf(lj_log, "Particle %i at (%.5lg, %.5lg) should be moved by (%.5lg, %.5lg)\n", i, particle[i].xc, particle[i].yc, deltax[i], deltay[i]);
    }
    ...
    if (move == 0) 
    {
        free(move_part);
        free(move_molecule);
        free(molecule_refparticle);
        free(move_cluster);
        free(cluster_refparticle);
        free(deltax);
        free(deltay);
        return(0); 
    }

    fprintf(lj_log, "[wrap_particles_periodic_bc] %i particles and containing molecules to be moved\n", move);
    /* determine molecules to be moved */
    for (mol = 0; mol < ncircles; mol++) move_molecule[mol] = 0;
    
    for (i=0; i<ncircles; i++) if (move_part[i])
    {
        fprintf(lj_log, "[wrap_particles_periodic_bc] Particle %i at (%.5lg, %.5lg) will be moved by (%.5lg, %.5lg)\n", i, particle[i].xc, particle[i].yc, deltax[i], deltay[i]);

        mol = particle[i].molecule;
        move_molecule[mol] = 1;
        molecule_refparticle[mol] = i;
        fprintf(lj_log, "[wrap_particles_periodic_bc] Particle %i: molecule %i, ", i, mol);
        for (p=0; p<molecule[mol].npartners; p++)
        {
            move_molecule[molecule[mol].partner[p]] = 1;
            molecule_refparticle[molecule[mol].partner[p]] = i;
            fprintf(lj_log, "%i, ", molecule[mol].partner[p]);
        }
        fprintf(lj_log, "\n");
    }
    
    /* move molecules */
    for (mol=0; mol<ncircles; mol++) if (move_molecule[mol])
    {
        fprintf(lj_log, "[wrap_particles_periodic_bc] Moving molecule %i with %i particles\n", mol, molecule[mol].nparticles); 
        i = molecule_refparticle[mol];
        for (p=0; p<molecule[mol].nparticles; p++)
        {
            j = molecule[mol].particle[p];
            particle[j].xc += deltax[i];
            particle[j].yc += deltay[i];
            fprintf(lj_log, "[wrap_particles_periodic_bc] Moving particle %i to (%.5lg, %.5lg)\n", j, particle[j].xc, particle[j].yc);
        }
    }
    
    /* determine clusters to be moved */
    for (clust = 0; clust < ncircles; clust++) move_cluster[clust] = 0;
    
    for (i=0; i<ncircles; i++) if (move_part[i])
    {
        clust = particle[i].cluster;
        move_cluster[clust] = 1;
        cluster_refparticle[clust] = i;
    }
    
    /* move clusters */
    for (clust=0; clust<ncircles; clust++) if (move_cluster[clust])
    {
        cluster[clust].xg += deltax[cluster_refparticle[clust]];
        cluster[clust].yg += deltay[cluster_refparticle[clust]];
    }
...
int wrap_particles_sphere_bc(t_particle particle[NMAXCIRCLES], t_molecule molecule[NMAXCIRCLES], t_cluster cluster[NMAXCIRCLES])
/* wrap particles for certain boundary conditions */
{
    int i, j, k, p, mol, clust, move = 0;
    short int *move_part, *move_molecule, *move_cluster, *molecule_refparticle, *cluster_refparticle;
...
/* initialise absorbing discs */
{
    ...
    switch (ABSORBER_PATTERN) {
        ...
        case (AP_DODECA):
        {
            ...
            for (k=0; k<3; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/3.0;
                absorber[nabsorb].yc = alpha;
                nabsorb++;
            }
            ...
            for (k=0; k<3; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/3.0 + gamma;
                absorber[nabsorb].yc = beta;
                nabsorb++;
                absorber[nabsorb].xc = (double)k*DPI/3.0 - gamma;
                absorber[nabsorb].yc = beta;
                nabsorb++;
            ...
            for (k=0; k<3; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/3.0 + PI/3.0 + gamma;
                absorber[nabsorb].yc = PI - beta;
                nabsorb++;
                absorber[nabsorb].xc = (double)k*DPI/3.0 + PI/3.0 - gamma;
                absorber[nabsorb].yc = PI - beta;
                nabsorb++;
            ...
            for (k=0; k<3; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/3.0 + PI/3.0;
                absorber[nabsorb].yc = PI - alpha;
                nabsorb++;
            }
        ...
        case (AP_ICOSA):
        {
            ...
            for (k=0; k<5; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/5.0;
                absorber[nabsorb].yc = alpha;
                nabsorb++;
            }
            ...
            for (k=0; k<5; k++)
            {
                absorber[nabsorb].xc = (double)k*DPI/5.0 + PI/5.0;
                absorber[nabsorb].yc = PI - alpha;
                nabsorb++;
            }
...
int wrap_particles(t_particle particle[NMAXCIRCLES], t_molecule molecule[NMAXCIRCLES], t_cluster cluster[NMAXCIRCLES])
/* wrap particles, molecules, clusters, for certain boundary conditions */
{
    switch(BOUNDARY_COND) {
        case (BC_PERIODIC): 
        {
            return(wrap_particles_periodic_bc(particle, molecule, cluster));
        }
        case (BC_SPHERE): 
        {
            return(wrap_particles_sphere_bc(particle, molecule, cluster));
        }
        default: /* TODO */
            return(0);
    }
...
void draw_frame_2d(int i, int plot, int bg_color, int ncollisions, int traj_position, 
                int traj_length, 
...
//     else if (!TRACER_PARTICLE) blank();
    if ((REACTION_DIFFUSION)&&(ncollisions > 0)) draw_collisions(collisions, ncollisions, hashgrid);
...
//     if (DRAW_OBSTACLE_LINKS) old_draw_obstacle_triangles(obstacle);
    if (TRACER_PARTICLE) draw_trajectory(trajectory, hashgrid, traj_position, traj_length, particle, cluster, tracer_n, plot);
    if (ADD_ABSORBERS) draw_absorbers(absorber); 
    ...
    if (PLOT_CURRENTS) draw_current_plot(currents, i); 
...
void draw_frame_3d(int i, int plot, int bg_color, int ncollisions, int traj_position, 
                int traj_length, 
                int wall, double pressure[N_PRESSURES], double pleft, double pright, 
                double *currents, 
                int *particle_numbers, short int refresh, 
                ...
                t_group_data *group_speeds, t_group_segments *segment_group, 
                t_belt *conveyor_belt, int *tracer_n,
                t_otriangle otriangle[NMAX_TRIANGLES_PER_OBSTACLE*NMAXOBSTACLES], t_ofacet ofacet[NMAXOBSTACLES], t_lj_sphere wsphere[NX_SPHERE*NY_SPHERE], 
                t_absorber absorber[NMAX_ABSORBERS])
/* draw a movie frame */
{
//     static int first;
//     
//     if (first)
//     {
//         if (ADD_ABSORBERS) draw_absorbers_sphere(absorber, wsphere); 
//         first = 0;
//     }
